...
$BranchName = "main"
$VersionApk = "Licznik-v3.7-2906261215.apk"
...
if ($Main -notmatch 'VERSION_NAME = "3\.7 - 2906261215"') {
    Fail "MainActivity.java nie ma wersji 3.7."
}
...
if ($Workflow -notmatch [regex]::Escape($VersionApk)) {
    Fail "Workflow nie publikuje APK 3.7."
}
...
if ($UseCurrentRepo) {
    ...
    $PublishDir = Join-Path $TempBase ("Licznik_publish_v37_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
    ...
    robocopy $Root $PublishDir /E /XD ".git" ".gradle" "build" "app\build" "Licznik_publish_v37_*" /XF "*.apk" | Out-Host
...
git commit -m "Licznik 3.7: poprawki mapy kompasu i POI"
