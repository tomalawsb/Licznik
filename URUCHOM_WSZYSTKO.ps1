...
$BranchName = "main"
$VersionApk = "Licznik-v3.8-2906261235.apk"
...
if ($Main -notmatch 'VERSION_NAME = "3\.8 - 2906261235"') {
    Fail "MainActivity.java nie ma wersji 3.8."
}
...
if ($Workflow -notmatch [regex]::Escape($VersionApk)) {
    Fail "Workflow nie publikuje APK 3.8."
}
...
if ($UseCurrentRepo) {
    ...
    $PublishDir = Join-Path $TempBase ("Licznik_publish_v38_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
    ...
    robocopy $Root $PublishDir /E /XD ".git" ".gradle" "build" "app\build" "Licznik_publish_v38_*" /XF "*.apk" | Out-Host
...
git commit -m "Licznik 3.8: mapa bez wymuszania pozycji auto wyszukiwarka ikona"
