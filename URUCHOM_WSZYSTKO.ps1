...
$BranchName = "main"
$VersionApk = "Licznik-v3.9-2906261305.apk"
...
if ($Main -notmatch 'VERSION_NAME = "3\.9 - 2906261305"') {
    Fail "MainActivity.java nie ma wersji 3.9."
}
...
if ($Workflow -notmatch [regex]::Escape($VersionApk)) {
    Fail "Workflow nie publikuje APK 3.9."
}
...
if ($UseCurrentRepo) {
    ...
    $PublishDir = Join-Path $TempBase ("Licznik_publish_v39_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
    ...
    robocopy $Root $PublishDir /E /XD ".git" ".gradle" "build" "app\build" "Licznik_publish_v39_*" /XF "*.apk" | Out-Host
...
git commit -m "Licznik 3.9: parametry kalorii w ustawieniach"
